...
import (
	...
	dynamiclister "openebs.io/metac/dynamic/lister"
)
...
type SharedIndexInformer interface {
	...
	// RemoveEventHandlers removes all event handlers added through this
	// instance of SharedIndexInformer. It does not affect other handlers
	// on the same underlying shared informer.
	//
	// This is necessary because the underlying shared informer may continue
	// running if others are using it, so you should remove the handlers you
	// added when you're no longer interested in receiving events.
	RemoveEventHandlers()
}
...
// If you add any event handlers with this interface, you should
// arrange to call RemoveEventHandlers() when you want to stop
// receiving events.
func (ri *ResourceInformer) Informer() SharedIndexInformer {
	return ri.informerWrapper
}

// Lister returns a shared, dynamic lister that's analogous to the
// static listers generated for static types.
func (ri *ResourceInformer) Lister() *dynamiclister.Lister {
	return ri.sharedResourceInformer.lister
}

// Close marks this ResourceInformer as unused, allowing the underlying
// shared informer to be stopped when no users are left.
