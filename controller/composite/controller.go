...
func newParentController(
	...
	parentClient, err := dynClient.Resource(
	...
	pc = &parentController{
		...
		finalizer: &finalizer.Manager{
			Name:    "metac.openebs.io/compositecontroller-" + cc.Name,
...
func (pc *parentController) Start() {
	...
	go func() {
		...
		if !k8s.WaitForCacheSync(pc.parentResource.Kind, pc.stopCh, syncFuncs...) {
			...
			glog.Warningf(
				"%v CompositeController cache sync never finished", pc.parentResource.Kind,
			)
			return
		}
...
func (pc *parentController) processNextWorkItem() bool {
	...
	if err != nil {
		utilruntime.HandleError(fmt.Errorf(
			"failed to sync %v %q: %v", pc.parentResource.Kind, key, err),
		)
...
func (pc *parentController) onChildAdd(obj interface{}) {
	...
	if controllerRef := metav1.GetControllerOf(child); controllerRef != nil {
		...
		glog.V(4).Infof(
			"%v %v/%v: child %v %v created or updated",
	...
	glog.V(4).Infof(
		"%v: orphan child %v %s created or updated",
		pc.parentResource.Kind,
		child.GetKind(),
		child.GetName(),
	)
...
// makeSelector builds a label selector based on couple of parent
// fields i.e. generateSelector or selector
	...
	if pc.cc.Spec.GenerateSelector != nil && *pc.cc.Spec.GenerateSelector {
		...
		if err != nil {
			return nil, fmt.Errorf(
				"can't get label selector from %v %v/%v",
				pc.parentResource.Kind,
				parent.GetNamespace(),
				parent.GetName(),
			)
		...
		if len(labelSelector.MatchLabels) == 0 && len(labelSelector.MatchExpressions) == 0 {
			return nil, fmt.Errorf(
				".spec.selector must have either matchLabels, matchExpressions, or both",
			)
		}
	}
...
func (pc *parentController) canAdoptFunc(parent *unstructured.Unstructured) func() error {
	return k8s.RecheckDeletionTimestamp(func() (metav1.Object, error) {
		...
		if fresh.GetUID() != parent.GetUID() {
			return nil, fmt.Errorf(
				"original %v %v/%v is gone: got uid %v, wanted %v",
				pc.parentResource.Kind,
				parent.GetNamespace(),
				parent.GetName(),
				fresh.GetUID(),
