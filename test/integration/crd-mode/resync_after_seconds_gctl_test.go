...
func TestResyncAfterSecondsViaGctl(t *testing.T) {
	f := framework.NewIntegrationTester(t)
	defer f.TearDown()

	watchName := "my-watch"
	namespaceName := "ns-rasvg"
	...
	syncHook := f.ServeWebhook(func(body []byte) ([]byte, error) {
		...
		if req.Watch.Object["status"] == nil {
			...
			watchStatus := req.Watch.Object["status"]
	...
	result, err := f.Test(
		[]framework.TestStep{
				...
				Apply: framework.Apply{
					State: &unstructured.Unstructured{
						Object: map[string]interface{}{
							...
							"metadata": map[string]interface{}{
								"name": "raswatches.integration.test.io",
							},
				...
				Apply: framework.Apply{
					State: &unstructured.Unstructured{
						Object: map[string]interface{}{
							...
							"metadata": map[string]interface{}{
								"name":      "ras-watch-resync-after-seconds-gctl",
								"namespace": namespaceName,
							},
							"spec": map[string]interface{}{
			...
			framework.TestStep{
				Name: "assert-watch-has-elapsedseconds-lte-1.0",
